...
首次 `mimi` 会执行幂等初始化：创建权限为 `0700` 的数据目录、`0600` 的策略与 Connector 配置和本机数据库，并把发布包内的 Connector 路径解析到当前安装位置。macOS 默认只启用无界面的 System Connector 和 action-only Desktop Connector；Calendar、Mail、Messages、Contacts、Notes、Shortcuts、Browser、Screen、Voice、Radar 等能力都需要用户显式启用。发布构建会记录 Git commit、dirty 状态和运行文件摘要；`mimi daemon doctor` 只读检查模型凭证是否已配置、后台状态、Connector readiness、容量阈值和构建漂移，不读取邮件、消息或屏幕，也不会自动修改工作树。
...
个人 QQ 使用事件绑定的 `PersonalMessageHub → ComputerManager/CUA` 窄路由：只读取当前唯一后台窗口，按界面
账号与会话指纹复核目标，保护 owner 前台活动和已有草稿，并在唯一一次 Return 后重新
观察同一会话；缺少正式入站观察器或 Computer 实机 readiness 时仍保持 unavailable，
不会退回 Browser、外部 Shell Skill 或其他消息后端。更完整的边界见 [Connector 文档](docs/CONNECTORS.md)。
