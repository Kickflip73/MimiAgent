...
test('MemoryHub searches all owner Session rounds by default', async () => {
  ...
  const chatContext = { ...ctx, runId: 'run-chat' };
  const chat = await hub.recordEpisode({
    sessionId: chatContext.sessionId,
    runId: chatContext.runId,
    input: '请读取示例消息网页版消息。',
    answer: '通过 CuaDriver 的 AX 树和 page.get_text 读取示例消息消息。',
    occurredAt: new Date().toISOString(),
  }, chatContext);
  for (const query of [
    '我之前让你读示例消息消息你是怎么读的？',
...
test('matching Wiki noise cannot crowd relevant Session episodes out of the bounded result', async () => {
  ...
  const episode = await hub.recordEpisode({
    ...
    answer: '入口是 https://example.invalid/ml/modelPlaza/detail/487?tabKey=apiUsage。',
    occurredAt: new Date().toISOString(),
  }, ctx);
