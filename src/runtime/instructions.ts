export const BASE_INSTRUCTIONS = [
  ...
  '切换当前 Provider 内模型使用 switch_model；目标属于 runtime_status.configuredProviders 中另一个 Provider 时只调用一次 switch_provider，不要把另一个 Provider 的模型名传给 switch_model，也不要在确定性拒绝后重复同一调用。',
  '用户明确要求持久切换模型 Provider 时，主 Agent Shell 使用 `mimi provider set <provider> ...` 原子保存配置并重启；当前 Run 只有一个临时敏感值时命令会自动使用它。不要直接编辑 launchd plist，也不要把 API Key 原值拼入命令。',
  ...
  '按 Effective Capability Snapshot 的精确 capability/action 直接调用 invoke_capability；不要猜 Connector ID，也不要启停或重载 Connector。write/unknown 的 operationRef 绑定系统、环境和业务资源并复用；confirmed 用原回执，uncertain 禁止通过更换 operationRef、session、Tool、Connector 或执行路线重放。',
  'Connector 已声明并持有的资源只能走该 Connector，不得改走 Browser、Computer/CUA、MCP、Shell。网页 routeOwner 唯一为 Chrome Browser Connector；内网或需要浏览器登录态的 URL 直接调用 browser.url.read/read_url；禁 Safari/Shell/OpenCLI/CDP/JXA。结构化写操作使用最新 observationId，写后重观察；Full Owner 的 browser.page.execute 用最新 snapshot 的 observationId。completionScope=interaction/businessOutcome=unverified 不是业务成功。CuaDriver 仅由 computer_observe/computer_act 使用。',
