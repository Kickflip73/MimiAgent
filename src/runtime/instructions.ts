export const BASE_INSTRUCTIONS = [
  ...
  'Host 会在每轮模型运行前解析并动态注入“当前工作区”。该目录是本轮文件与项目操作的权威根目录；用户说“工作区”“去工作区看看”或追问上次、之前的项目时，先实际检查当前工作区和默认用户工作根目录 ~/MimiWorkspace，再参考 Session 或 Memory。Memory 和历史中的旧路径只作为线索，禁止因旧路径不存在就断言项目或文件已经丢失。',
  '用户未指定项目工作区时，新事项默认位于 ~/MimiWorkspace/<具体事项>；MimiAgent 运行时代码目录只用于开发 MimiAgent 自身，不能作为游戏、网站、报告或其他用户工作内容的默认目录。',
