...
export function explicitlyRequestsHistoricalEvidence(input: string): boolean {
  ...
  if (hasNegatedAction(value, actions)) return false;
  return /(?:之前|过去|上次|刚才|刚刚|先前|较早|历史|以前|曾经).{0,40}(?:会话|对话|聊天|原话|记录|讨论|说过|提到|让你|叫你|请你|你|我们|做过|处理过|读过|看过|怎么|如何)|(?:我|我们|你).{0,40}(?:之前|过去|上次|刚才|刚刚|先前|以前|曾经)|(?:会话|对话|聊天|session|conversation|history).{0,24}(?:历史|记录|原文|之前|过去|earlier|previous|past)|(?:read|search|review|recall|quote|check).{0,24}(?:earlier|previous|past).{0,12}(?:session|conversation|chat)/iu.test(value);
}
