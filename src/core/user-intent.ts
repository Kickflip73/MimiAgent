...
export type UserCapabilityDisclosure = 'status' | 'runtime' | 'session' | 'web' | 'lightweight' | 'full';
...
export function capabilityDisclosureForInput(input: string): UserCapabilityDisclosure {
  ...
  const runtimeIntent = /(?:(?:为什么|为何|怎么|怎样).{0,24}(?:Computer\s*Use|CuaDriver|电脑操作|工具|能力).{0,20}(?:不可用|不能用|用不了|没有|未启用|未注册|未暴露)|(?:Computer\s*Use|CuaDriver|电脑操作|工具|能力).{0,24}(?:为什么|为何|怎么|怎样)?.{0,12}(?:不可用|不能用|用不了|没有|未启用|未注册|未暴露)|(?:当前|现在).{0,12}(?:有哪些|什么|哪些).{0,8}(?:工具|能力)(?:可用)?)/iu.test(value);
  if (runtimeIntent) return 'runtime';
  ...
  const directExperiment = /(?:你|请|帮我|麻烦你?).{0,12}(?:试(?!着)|测试|验证|探测|复现)(?:一下|下|看看|看)?|\b(?:(?:please|you|can you|could you|would you)\s+)(?:try|test|verify|probe|reproduce)\b|\b(?:try|test|verify|probe|reproduce)\s+(?:it|this|that)\b/iu.test(value);
  const directExecution = /(?:你|请|帮我|麻烦你?).{0,24}(?:启用|打开|操作|控制|发送|回复|读取|查看|看看|查下|处理|运行|执行|使用.{0,12}(?:去|来)(?:操作|查看|处理))|\b(?:can|could|would|will)\s+you\s+(?:enable|open|operate|control|send|reply|read|check|handle|run|use)\b/iu.test(value);
  const informational = /(?:如何|怎么|怎样|为什么|解释|介绍|说明|什么是|区别|原理|含义|优缺点)|\b(?:how|why|what|explain|describe|introduce)\b/iu.test(value);
  if (directExperiment || directExecution
    || (concreteArtifact && (strongAction || inspectAction))
