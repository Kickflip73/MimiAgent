...
`auto` retrieval 的零 Key Provider 是 direct BGE q8：`onnx-community/bge-small-zh-v1.5-ONNX` revision `9507db33464b5da99a532ac26b2a251767cbc62b`，运行时精确锁定 `onnxruntime-node@1.24.3` 与 `@huggingface/tokenizers@0.1.3`。ONNX 主文件 SHA-256 为 `99a6e522710c00220c89f8c52e0cc5aa09d4cbb1c34c0e932eab3a9dfdc65df3`，外部权重为 `952623481ca8beea884e3d3c9ecaf8a3c7bf1d0c21de29e970cd31af9d37a90b`；其余三个 tokenizer/config 资产同样由 manifest 固定大小和摘要。资产只缓存在 `<dataRoot>/memory/models/bge-small-zh-v1.5-q8/<revision>/`，目录 `0700`、文件 `0600`，写入采用校验后原子替换。启动、查询与普通增量同步禁止下载；只有显式 `/memory reindex` 允许取得或修复固定资产。缺失、损坏、下载失败或不支持的平台都只改变 Embedding diagnostics，不能阻塞 Runtime 启动或词法查询。只有显式 `MIMI_EMBEDDING_API_KEY` 才改用远程 OpenAI-compatible Provider，不复用对话 Provider Key。

2026-08-05 Darwin arm64、32 个互不重复自然问题/80 个文档的离线串行选型基准记录 direct BGE 模型 23.180 MiB、完整 runtime install 211.675 MiB、warm query p95 2.111 ms、RSS 增量 118.61 MiB。E5 int8/q8 因模型与内存成本显著更高且中译英桶仍失效，Xenova v2 BGE WASM 因相近质量下 p95 27.523 ms、RSS 增量 326.89 MiB，均不作为默认。direct BGE 自身也不是可靠跨语模型：该小样本英译中 R@10 为 0%、中译英 R@10 为 50%，因此结构化/BM25 和证据不足语义必须保留。

自动检索最多处理 20 张候选/2400 tokens，随后 L3 Assembler 按默认 900-token 预算组成四类上下文，不再按固定卡片数截断。`source_receipts`、`suppressions` 和 `decision_events` 是不可随 reindex 删除的控制真相；页面、FTS、vec0 和 links 是可重建派生数据。`knowledge/sources/` 对 MemoryHub 只读。
...
终端输入支持 `@image:相对路径`、`@file:相对路径`、`@audio:相对路径` 和 `@video:相对路径`，含空格时可使用引号；已持久化的同 Session 原图可用 `@media:media-evidence:sha256:<digest>` 显式引用。只允许 `local-cli + owner` 经受控字段提交附件或媒体引用；外部来源、`payload.attachments`/保留引用字段注入、未知字段、绝对/控制字符名称、符号链接、工作区逃逸、声明 kind 与检测 MIME 不一致以及摘要变化均在 Event ingest 或模型请求前失败关闭。每个 Session 首次绑定一个物理 realpath，Daemon Event 只保存 opaque `workspaceId` 和有界 Evidence ID；受控 registry 在 Host 内解析工具与 attachment root，不把私人绝对路径或物化媒体写入 SQLite。
...
是 unit/adapter fixture，不是 live 图片 Provider 验收。

普通 CLI/Daemon Conversation 另有显式 `@media:<MediaEvidence.id>` 路径。ChatClient 从 owner
正文拆出引用并通过保留的 `referencedMediaEvidenceIds` Event 字段传递；自然语言 prompt 与
canonical Session transcript 不携带 data URL。Dispatcher 只接受绑定 Session 的 `local-cli + owner`
Event，并把引用作为本轮冻结的 `imageInput` requirement；pipeline 在同一 profile/workspace/trust
下读取 Session Evidence、验证 CAS 摘要，随后只为 Provider 请求构造临时 `input_image`。进程重启
后复用同一 Session JSON 与 artifact store 即可恢复；新附件和引用合计最多 8 项，既有 inline
附件与引用图片的 decoded bytes 合计最多 20 MiB，所有配额在分配 base64 前检查。

跨 Session、profile、workspace、trust、不存在/非图片/篡改 ref 或不支持 `imageInput` 的 route
均在 Provider I/O 前失败关闭。Host 在模型输入 factory 和 reference materializer 之前查询
completed execution receipt，因此 Task 安全回放即使 artifact 已丢失或损坏也不读 CAS、不调用
...
`WIKI.md` 的 YAML policy 控制 prefer-existing、SourceRef、canonicalKey、inferred-active、页面/来源上限和需要关系的页面类型；Markdown 正文给维护 Agent 定义 Ingest/Query/Lint 纪律，但不能扩大 scope、trust 或工具权限。确定性 Lint 可经 `lint-repair` Revision 修复 envelope、canonicalKey 和 inferred-active；语义维护使用有界 merge、supersede、link、move 与 refresh 工具形成 receipt 闭环。查询只读，不在热路径偷偷刷新 stale 来源；`/memory refresh` 显式重编译并保留旧 revision。查询优先返回 private/workspace Wiki，只有 Wiki 不足时才补充 Session episode/raw evidence；SQLite FTS5/BM25 为词法基线，零 Key 本地或显式专用 Key 远程 Embedding vector 以 RRF 合并，任一 Provider/Vec 故障都不能阻塞词法通道。
...
Provider Key；其模型还必须在当前 model registry 精确注册，并以该 target 的真实 capability
通过冻结 WorkUnit 要求，不能按 Provider 名或任意模型字符串推定媒体/fileInput 支持。未注册
或不兼容的 backup 在任何 Provider I/O 前从候选中移除，公开 `providerRoute` 也走相同精确绑定，
不再绕过 ModelResolver。主 Daemon、每个 Session actor 和 isolated worker 都显式构造同一主备
