...
- Kernel 是全局控制面，不是全局工作区。owner CLI Event 携带本次启动目录；显式项目路径或唯一项目名优先，否则 Dispatcher 默认采用该启动目录。只有明确的新独立项目不适合当前目录，或没有可用的启动/Session 工作区时，才选择 `~/MimiWorkspace/<具体事项>`；Session actor 随后用解析出的目录重建完整运行时。
